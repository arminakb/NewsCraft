...
SCHEMA_HEAD_MIGRATION = Path("alembic/versions/0026_remove_operator_sessions.py")
...
def test_application_schema_head_matches_latest_migration():
    ...
    assert SCHEMA_HEAD == "0026_remove_operator_sessions"
    assert 'revision = "0026_remove_operator_sessions"' in source
    assert 'down_revision = "0025_operator_sessions"' in source

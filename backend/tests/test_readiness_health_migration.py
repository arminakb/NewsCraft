...
SCHEMA_HEAD_MIGRATION = Path("alembic/versions/0023_source_soft_deletion.py")
...
def test_application_schema_head_matches_latest_migration():
    ...
    assert SCHEMA_HEAD == "0023_source_soft_deletion"
    assert 'revision = "0023_source_soft_deletion"' in source
    assert 'down_revision = "0022_article_canonical_classification"' in source

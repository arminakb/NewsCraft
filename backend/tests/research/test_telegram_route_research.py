...
def test_off_policy_stores_no_research_profile():
    value = TelegramResearchPolicyInput.model_validate(
        {"research_mode": "off", "research_provider_profile_id": None}
    )
...
def _process_probe(*, mode: str, complete: bool, manual_succeeded: bool = False):
    ...
    generation_profile = AIProviderProfile(
        id=uuid4(), name="Codex", provider_type="codex", default_model="gpt-5.4",
        secret_ref=None, settings=default_codex_provider_settings().model_dump(mode="json"),
        enabled=True,
    )
    research_profile = AIProviderProfile(
        id=uuid4(), name="Research fake", provider_type="fake", default_model="fake-v1",
        secret_ref=None, settings={}, enabled=True,
    )
    route = AutomationRoute(
        id=uuid4(), source_id=uuid4(), destination_id=uuid4(), brand_profile_id=uuid4(),
        prompt_template_version_id=uuid4(), ai_provider_profile_id=generation_profile.id,
        research_mode=mode,
        content_filters=(
            {} if mode == "off"
            else {"research_provider_profile_id": str(research_profile.id)}
        ),
        attribution_policy="preserve", custom_footer=None, retry_policy={},
    )
    story = Story(
        id=uuid4(), title="Telegram story", status="inbox", primary_language="fa",
    ...
    source_item = SourceItem(
        id=uuid4(), source_id=route.source_id, content_item_id=content_item.id,
        external_id_raw="source:1", external_id_norm="source:1", content_text_raw="source",
    )
    ...
    snapshot = StoryEvidenceSnapshot(
        ...
        source_url="https://one.example/item", content_text=text, content_sha256=digest,
        authors=[], snapshot_metadata={"is_primary": complete}, captured_at=now,
    )
    snapshots = [snapshot]
    if complete:
        other_text = "y" * 500
        ...
        snapshots.append(
            StoryEvidenceSnapshot(
                ...
                source_url="https://two.example/item", content_text=other_text,
                content_sha256=other_digest, authors=[], snapshot_metadata={}, captured_at=now,
            )
        )
    link = StoryEvidenceLink(
        story_revision_id=revision.id, evidence_snapshot_id=snapshot.id,
        claim_key="telegram.source", relationship="supports",
    )
    prompt = PromptTemplateVersion(
        id=route.prompt_template_version_id, prompt_template_id=uuid4(), version=1,
        system_template="Locked", user_template=(
        ...
        output_schema=TelegramRewriteOutput.model_json_schema(), checksum_sha256="a" * 64,
        is_active=True,
    )
    brand = BrandProfile(id=route.brand_profile_id, name="Brand", output_language="fa", tone="neutral")
    destination = Destination(
        id=route.destination_id, name="Destination", platform="telegram",
        target_ref="@destination", secret_ref="DESTINATION_TOKEN", enabled=True,
        health_status="healthy", settings={},
    )
    dispatch = AutomationDispatch(
        ...
        source_message_ids=[1], dispatch_kind="live", status="captured", created_at=now,
    )
    values = [
        route, story, revision, source_item, content_item, *snapshots, prompt, brand,
        generation_profile, research_profile, destination,
    ]
    if manual_succeeded:
        values.append(
            ResearchRun(
                id=uuid4(), story_id=story.id, requested_mode="manual",
                provider_profile_id=research_profile.id, status="succeeded",
                query_budget=4, page_budget=8, time_budget_seconds=120,
                result_story_revision_id=revision.id, created_at=now, finished_at=now,
            )
        )
    ...
    return SimpleNamespace(
        handler=handler, session=session, job=job, provider=provider, dispatch=dispatch,
...
async def test_off_mode_enters_generation_without_research_enqueue(monkeypatch):
    ...
    with pytest.raises(ProbeStop):
        await probe.handler(
            probe.job, JobContext(session=probe.session, providers=SimpleNamespace())
        )
...
async def test_manual_mode_reviews_without_auto_research_then_resumes_once():
    ...
    with pytest.raises(NeedsReviewJobError, match="Manual research"):
        await pending.handler(
            pending.job, JobContext(session=pending.session, providers=SimpleNamespace())
        )
    assert pending.provider.request is None

    resumed = _process_probe(mode="manual", complete=False, manual_succeeded=True)
    with pytest.raises(ProbeStop):
        await resumed.handler(
            resumed.job, JobContext(session=resumed.session, providers=SimpleNamespace())
        )
...
async def test_auto_incomplete_enqueues_one_exact_research_job_before_generation(monkeypatch):
    ...
    assert calls[0]["idempotency_key"].endswith(
        f":{probe.research_profile.id}:fake-v1:auto_if_incomplete:standard"
    )
...
async def test_failed_research_leaves_dispatch_review_without_continuation_or_publish(monkeypatch):
    ...
    with pytest.raises(NeedsReviewJobError):
        await build_research_story_handler(
            lambda _profile: ObservingBackend(session, output)
        )(job, JobContext(session=session, providers=SimpleNamespace()))
    persisted = next(
        value for value in session.values
        if isinstance(value, AutomationDispatch) and value.id == dispatch.id
    )

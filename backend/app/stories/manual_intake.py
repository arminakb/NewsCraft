...
MAX_MANUAL_RESPONSE_BYTES = 5 * 1024 * 1024

type Resolver = Callable[[str], Awaitable[Sequence[str]]]


class ManualIntakeFetchError(RuntimeError):
    """A fixed-message boundary failure safe to map into job state."""
...
def _normalized_host(value: str) -> str:
    ...
    try:
        ...
        raise ManualIntakeFetchError("Manual URL request rejected") from exc
...
class _PinnedAsyncTransport(httpx.AsyncBaseTransport):
    ...
    async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
        ...
        try:
            ...
            raise ManualIntakeFetchError("Manual URL request failed") from exc
...
class ManualIntakeHttpClient:
    def __init__(
        ...
        *,
        ...
        max_redirects: int = MAX_MANUAL_REDIRECTS,
        max_response_bytes: int = MAX_MANUAL_RESPONSE_BYTES,
    ) -> None:
    ...
    async def __aenter__(self) -> ManualIntakeHttpClient:
    ...
    async def _request(self, method: str, url: str, **kwargs: Any) -> httpx.Response:
        ...
        for redirect_count in range(self._max_redirects + 1):
            current_url = await self._validate_public_url(current_url)
            ...
            if redirect_count == self._max_redirects:
                raise ManualIntakeFetchError("Too many manual URL redirects")
    ...
    async def _validate_public_url(self, value: str) -> str:
        try:
        ...
        try:
            ...
            try:
                ...
                raise ManualIntakeFetchError("Manual URL request rejected") from exc
            if not answers:
                raise ManualIntakeFetchError("Manual URL request rejected") from None
            try:
        ...
        if not all(address.is_global for address in addresses):
            raise ManualIntakeFetchError("Manual URL request rejected")
        pinned_addresses = tuple(str(address) for address in addresses)
        self._pinned_backend.pin(host, pinned_addresses)
            ...
            **kwargs,
            ...
            if content_length is not None:
                try:
                    ...
                    raise ManualIntakeFetchError("Manual URL response is too large") from exc
                if declared_length > self._max_response_bytes:
                    raise ManualIntakeFetchError("Manual URL response is too large")

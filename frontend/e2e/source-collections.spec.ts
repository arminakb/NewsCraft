...
test("Source Collection navigation stays bounded with dozens of collections", async ({ page }) => {
  ...
  await expect(contextMenu.getByRole("menuitem", { name: "Edit details" })).toBeFocused()
  await expect(contextMenu.getByRole("menuitem", { name: "Rename" })).toHaveCount(0)
  await page.keyboard.press("Escape")
  ...
  await contextMenu.getByRole("menuitem", { name: "Edit details" }).click()
  const editDetailsDialog = page.getByRole("dialog", { name: "Edit Source Collection" })
  await expect(editDetailsDialog.getByLabel("Name")).toBeFocused()
  await editDetailsDialog.getByRole("button", { name: "Cancel" }).click()
  await expect(visualDeskRow).toBeFocused()

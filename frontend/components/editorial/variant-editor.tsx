...
export function VariantEditor({ revision, availableProviders = [], onSave, onApprove, onReject, onRegenerate, onReload, onDirtyChange, externalPending = false }: { revision: VariantRevision; availableProviders?: AIProviderOption[]; onSave?: (input: SaveInput) => Promise<VariantRevision>; onApprove?: (input: ApproveInput) => Promise<VariantRevision>; onReject?: (input: { revisionId: string; expectedContentHash: string; reason: string }) => Promise<VariantRevision>; onRegenerate?: (input: { variantId: string; providerProfileId: string; instruction: string | null }) => Promise<JobAccepted>; onReload?: () => Promise<void> | void; onDirtyChange?: (dirty: boolean) => void; externalPending?: boolean }) {
  ...
  return <section aria-labelledby="variant-editor-heading" className="min-w-0 space-y-4"><h2 id="variant-editor-heading" className="text-lg font-semibold">Exact revision editor</h2>
    ...
    <details className="rounded-lg border p-3" open={validationErrors.length > 0}>
      <summary className="cursor-pointer font-medium">Advanced Telegram details{validationErrors.length ? " — validation blocker" : ""}</summary>
      <p className="mt-3 break-all text-xs text-muted-foreground">Loaded revision {revision.id} · hash {revision.contentHash}</p>
      <div className="mt-3 grid gap-2 text-sm sm:grid-cols-2"><div>Parse mode: {revision.content.parseMode}</div><div>Media policy: {revision.content.mediaPolicy}</div><div>Direction: {revision.content.direction}</div><div>Dry run: {revision.content.dryRun ? "yes" : "no"}</div><div>Source provenance: {revision.content.sourceUrl ?? "Operator-provided text"}</div><div>Evidence citations: {revision.evidenceMap.length}</div></div>
    </details>
    {dirty ? <div role="status" className="text-warning">Changes will create a pending review revision</div> : null}
    ...
    {formErrors.map((message) => <div key={message} role="alert" className="text-sm text-destructive">{message}</div>)}{validationErrors.map((result) => <div key={result.gate} role="alert" className="text-sm text-destructive">{result.gate}: {result.reason ?? "failed"}</div>)}
    ...
    {generationProviders.length ? <fieldset disabled={busy} className="grid gap-3 rounded-lg border p-3"><legend>Regenerate from exact evidence</legend><label>AI provider<select aria-label="AI provider" className="ms-2 rounded border p-2" value={providerId} onChange={(event) => setProviderId(event.target.value)}><option value="">Select provider</option>{generationProviders.map((provider) => <option key={provider.id} value={provider.id} disabled={Boolean(provider.unavailableReason)}>{provider.name} · {provider.providerType} · {provider.defaultModel ?? "default"}{provider.unavailableReason ? ` unavailable: ${provider.unavailableReason}` : ""}</option>)}</select></label><p className="text-sm text-muted-foreground">The active Telegram prompt is resolved when the job is queued.</p><label className="grid gap-1">Regeneration instruction<DirectionBoundary as="textarea" language={null} className="rounded border p-2" value={instruction} onChange={(event) => setInstruction(event.target.value)} /></label><Button disabled={!providerId || busy || !onRegenerate} onClick={() => void run(() => onRegenerate!({ variantId: revision.variantId, providerProfileId: providerId, instruction: instruction.trim() || null }), "Regeneration queued")}>Regenerate</Button></fieldset> : null}
    {error ? <div role="alert" className="space-y-2 text-destructive"><p>{error}</p>{error.startsWith("A newer") ? <div className="flex flex-wrap gap-2"><Button variant="outline" disabled={busy} onClick={() => void reloadLatest()}>Reload latest</Button><span>Your unsaved edits stay available to reapply.</span></div> : null}</div> : null}{stashed && JSON.stringify(form) !== JSON.stringify(stashed) ? <Button variant="outline" disabled={externalPending} onClick={() => { setForm(stashed); setStashed(null); setError(null) }}>Reapply my edits</Button> : null}{outcome ? <div role="status" className="text-success">{outcome}</div> : null}
  </section>
}

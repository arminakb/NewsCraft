...
export function VariantEditor({ revision, availableProviders = [], availablePromptVersions = [], onSave, onApprove, onReject, onRegenerate, onReload, onDirtyChange }: { revision: VariantRevision; availableProviders?: AIProviderOption[]; availablePromptVersions?: PromptVersionOption[]; onSave?: (input: SaveInput) => Promise<VariantRevision>; onApprove?: (input: ApproveInput) => Promise<VariantRevision>; onReject?: (input: { revisionId: string; expectedContentHash: string; reason: string }) => Promise<VariantRevision>; onRegenerate?: (input: { variantId: string; providerProfileId: string; platformPromptTemplateVersionId: string; instruction: string | null }) => Promise<JobAccepted>; onReload?: () => Promise<void> | void; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  return <section aria-labelledby="variant-editor-heading" className="min-w-0 space-y-4"><h2 id="variant-editor-heading" className="text-lg font-semibold">Exact revision editor</h2><p className="break-all text-xs text-muted-foreground">Loaded revision {revision.id} · hash {revision.contentHash}</p>
    <label className="grid gap-1"><span>Telegram message</span><textarea aria-label="Telegram message" dir={revision.content.direction} className="min-h-64 rounded-lg border p-3" value={form.body} onChange={(event) => setForm({ ...form, body: event.target.value })} /></label>
    <label className="grid gap-1"><span>Parse mode</span><select aria-label="Parse mode" className="rounded-lg border p-2" value={form.parseMode} onChange={() => setForm({ ...form, parseMode: "HTML" })}><option value="HTML">HTML</option></select></label>
    <fieldset className="space-y-2 rounded-lg border p-3"><legend>Telegram buttons</legend>{form.buttons.map((button, index) => <div key={index} className="grid gap-2 sm:grid-cols-[1fr_2fr_auto]"><input aria-label={`Button ${index + 1} text`} className="rounded border p-2" value={button.text} onChange={(event) => setForm({ ...form, buttons: form.buttons.map((item, itemIndex) => itemIndex === index ? { ...item, text: event.target.value } : item) })} /><input aria-label={`Button ${index + 1} URL`} className="rounded border p-2" value={button.url} onChange={(event) => setForm({ ...form, buttons: form.buttons.map((item, itemIndex) => itemIndex === index ? { ...item, url: event.target.value } : item) })} /><Button type="button" variant="outline" onClick={() => setForm({ ...form, buttons: form.buttons.filter((_, itemIndex) => itemIndex !== index) })}>Remove button {index + 1}</Button></div>)}<Button type="button" variant="outline" disabled={form.buttons.length >= 8} onClick={() => setForm({ ...form, buttons: [...form.buttons, { text: "", url: "" }] })}>Add button</Button></fieldset>
    <label className="grid gap-1"><span>Media asset assignments</span><input aria-label="Media asset assignments" className="rounded-lg border p-2" value={form.mediaAssetIds.join(", ")} onChange={(event) => setForm({ ...form, mediaAssetIds: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) })} /></label>
    ...
    <label className="grid gap-1"><span>Edit note</span><input className="rounded-lg border p-2" value={editNote} onChange={(event) => setEditNote(event.target.value)} /></label>
    ...
    <div className="flex flex-wrap gap-2"><Button disabled={!dirty || !valid || pending || !onSave} onClick={() => void (async () => { if (await run(() => onSave!({ variantId: revision.variantId, baseRevisionId: revision.id, baseContentHash: revision.contentHash, content: { body: form.body, parseMode: form.parseMode, buttons: form.buttons }, mediaAssetIds: form.mediaAssetIds, editNote: editNote.trim() }), "New pending review revision created")) setBaseline(form) })()}>Save new revision</Button><Button disabled={dirty || !valid || pending || validationErrors.length > 0 || revision.approvalState !== "pending_review" || !onApprove} onClick={() => void run(() => onApprove!({ revisionId: revision.id, expectedContentHash: revision.contentHash, note: null }), "Revision approved")}>Approve revision</Button></div>
    <label className="grid gap-1"><span>Rejection reason</span><textarea className="rounded-lg border p-2" value={rejectionReason} onChange={(event) => setRejectionReason(event.target.value)} /></label><Button variant="outline" disabled={dirty || pending || !rejectionReason.trim() || revision.approvalState !== "pending_review" || !onReject} onClick={() => void run(() => onReject!({ revisionId: revision.id, expectedContentHash: revision.contentHash, reason: rejectionReason.trim() }), "Revision rejected")}>Reject revision</Button>
    {generationProviders.length && prompts.length ? <fieldset className="grid gap-3 rounded-lg border p-3"><legend>Regenerate from exact evidence</legend><label>AI provider<select aria-label="AI provider" className="ms-2 rounded border p-2" value={providerId} onChange={(event) => setProviderId(event.target.value)}><option value="">Select provider</option>{generationProviders.map((provider) => <option key={provider.id} value={provider.id} disabled={Boolean(provider.unavailableReason)}>{provider.name} · {provider.providerType} · {provider.defaultModel ?? "default"}{provider.unavailableReason ? ` unavailable: ${provider.unavailableReason}` : ""}</option>)}</select></label><label>Telegram pack prompt<select aria-label="Telegram pack prompt" className="ms-2 rounded border p-2" value={promptId} onChange={(event) => setPromptId(event.target.value)}><option value="">Select prompt</option>{prompts.map((prompt) => <option key={prompt.id} value={prompt.id}>Version {prompt.version}</option>)}</select></label><label className="grid gap-1">Regeneration instruction<textarea className="rounded border p-2" value={instruction} onChange={(event) => setInstruction(event.target.value)} /></label><Button disabled={!providerId || !promptId || pending || !onRegenerate} onClick={() => void run(() => onRegenerate!({ variantId: revision.variantId, providerProfileId: providerId, platformPromptTemplateVersionId: promptId, instruction: instruction.trim() || null }), "Regeneration queued")}>Regenerate</Button></fieldset> : null}
    {error ? <div role="alert" className="space-y-2 text-red-700"><p>{error}</p>{error.startsWith("A newer") ? <div className="flex flex-wrap gap-2"><Button variant="outline" disabled={pending} onClick={() => void reloadLatest()}>Reload latest</Button><span>Your unsaved edits stay available to reapply.</span></div> : null}</div> : null}{stashed && JSON.stringify(form) !== JSON.stringify(stashed) ? <Button variant="outline" onClick={() => { setForm(stashed); setStashed(null); setError(null) }}>Reapply my edits</Button> : null}{outcome ? <div role="status" className="text-green-700">{outcome}</div> : null}
  </section>
}

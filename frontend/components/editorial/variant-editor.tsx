...
export function VariantEditor({ revision, availableProviders = [], availablePromptVersions = [], onSave, onApprove, onReject, onRegenerate, onReload, onDirtyChange, externalPending = false }: { revision: VariantRevision; availableProviders?: AIProviderOption[]; availablePromptVersions?: PromptVersionOption[]; onSave?: (input: SaveInput) => Promise<VariantRevision>; onApprove?: (input: ApproveInput) => Promise<VariantRevision>; onReject?: (input: { revisionId: string; expectedContentHash: string; reason: string }) => Promise<VariantRevision>; onRegenerate?: (input: { variantId: string; providerProfileId: string; platformPromptTemplateVersionId: string; instruction: string | null }) => Promise<JobAccepted>; onReload?: () => Promise<void> | void; onDirtyChange?: (dirty: boolean) => void; externalPending?: boolean }) {
  ...
  return <section aria-labelledby="variant-editor-heading" className="min-w-0 space-y-4"><h2 id="variant-editor-heading" className="text-lg font-semibold">Exact revision editor</h2><p className="break-all text-xs text-muted-foreground">Loaded revision {revision.id} · hash {revision.contentHash}</p>
    <label className="grid gap-1"><span>Telegram message</span><textarea aria-label="Telegram message" disabled={busy} dir={revision.content.direction} className="min-h-64 rounded-lg border p-3" value={form.body} onChange={(event) => setForm({ ...form, body: event.target.value })} /></label>
    ...
    <fieldset disabled={busy} className="space-y-2 rounded-lg border p-3"><legend>Telegram buttons</legend>{form.buttons.map((button, index) => <div key={index} className="grid gap-2 sm:grid-cols-[1fr_2fr_auto]"><input aria-label={`Button ${index + 1} text`} className="rounded border p-2" value={button.text} onChange={(event) => setForm({ ...form, buttons: form.buttons.map((item, itemIndex) => itemIndex === index ? { ...item, text: event.target.value } : item) })} /><input aria-label={`Button ${index + 1} URL`} className="rounded border p-2" value={button.url} onChange={(event) => setForm({ ...form, buttons: form.buttons.map((item, itemIndex) => itemIndex === index ? { ...item, url: event.target.value } : item) })} /><Button type="button" variant="outline" onClick={() => setForm({ ...form, buttons: form.buttons.filter((_, itemIndex) => itemIndex !== index) })}>Remove button {index + 1}</Button></div>)}<Button type="button" variant="outline" disabled={form.buttons.length >= 8} onClick={() => setForm({ ...form, buttons: [...form.buttons, { text: "", url: "" }] })}>Add button</Button></fieldset>
    ...
    <label className="grid gap-1"><span>Edit note</span><input disabled={busy} className="rounded-lg border p-2" value={editNote} onChange={(event) => setEditNote(event.target.value)} /></label>

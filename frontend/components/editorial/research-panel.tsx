...
export function ResearchPanel({ story, providers, run, onCompleted }: { story: StoryDetail; providers: AIProviderOption[]; run: ResearchRunDetail | null; onCompleted?: (runId: string) => void }) {
  ...
  return <section className="space-y-4" aria-label="Research controls">
    ...
    <label className="grid gap-1 text-sm font-medium"><span>Research note (optional)</span><DirectionBoundary as="textarea" language={null} className="min-h-20 rounded-lg border bg-background px-3 py-2" maxLength={500} value={queryHint} onChange={(event) => setQueryHint(event.target.value)} /></label>
...
function ResearchOutcome({ run }: { run: ResearchRunDetail }) {
  ...
  if (run.status === "succeeded") return <div className="space-y-2"><div>Research completed</div>{run.sources.map((source) => <div key={source.id} className="flex flex-wrap items-center gap-2"><DirectionBoundary as="span" language={null}>{source.title ?? "Fetched source"}</DirectionBoundary><a className="text-primary underline" href={source.url} target="_blank" rel="noreferrer">Open fetched source</a></div>)}</div>

...
export const STORY_INBOX_PAGE_SIZE = 100
export const MAX_STORY_SELECTION = 200
...
export function StoryInbox({ initialStories }: { initialStories?: StorySummary[] }) {
  ...
  const filters: StoryFilters = { search: search || undefined, editorialState: editorialState || undefined, completeness: completeness || undefined, limit: STORY_INBOX_PAGE_SIZE }
  useEffect(() => {
  ...
  const visible = useMemo(() => stories.slice(0, STORY_INBOX_PAGE_SIZE), [stories])
  const selectedIds = useMemo(() => Array.from(selected), [selected])

  useEffect(() => {
    if (!query.data && !linkedStory.data) return
    const page = query.data?.items ?? []
    setStories(boundedStoryPage(linkedStory.data ? [linkedStory.data, ...page] : page))
    if (query.data) {
  ...
  const invalidate = useCallback(async (id?: string) => { await client.invalidateQueries({ queryKey: ["stories"] }); if (id) await client.invalidateQueries({ queryKey: editorialQueryKeys.story(id) }) }, [client])
  const bulk = useMutation({ mutationFn: ({ ids, state }: { ids: string[]; state: "shortlisted" | "rejected" }) => bulkSetStoryEditorialState(ids, state), onSuccess: async (_result, { ids, state }) => { const changedSet = new Set(ids); if (editorialState && editorialState !== state) setStories((current) => current.filter((story) => !changedSet.has(story.id))); setSelected(new Set()); setActionError(null); await invalidate(); await Promise.all(ids.map((id) => client.invalidateQueries({ queryKey: editorialQueryKeys.story(id) }))) }, onError: (error) => setActionError(getApiErrorMessage(error)) })
  ...
  async function loadMore() {
    ...
    try {
      ...
      setStories(boundedStoryPage(next.items))
  ...
  const selectVisible = useCallback(() => {
    setSelected((current) => {
      ...
      for (const story of visible) {
        if (next.size >= MAX_STORY_SELECTION) break
        next.add(story.id)
      }
  ...
  const selectStory = useCallback((id: string, checked: boolean) => {
    setSelected((current) => {
      if (checked && (current.has(id) || current.size >= MAX_STORY_SELECTION)) return current
      if (!checked && !current.has(id)) return current
      const next = new Set(current)
  ...
  const toggleStory = useCallback((id: string) => setOpenStory((current) => current === id ? null : id), [])
  const researchStoryById = useCallback((id: string) => setResearchStory(id), [])
  const changeStoryState = useCallback(async (id: string, state: "shortlisted" | "rejected") => {
    try {
      await setStoryEditorialState(id, state)
      if (editorialState && editorialState !== state) setStories((current) => current.filter((item) => item.id !== id))
      setActionError(null)
      await invalidate(id)
    } catch (error) {
      setActionError(getApiErrorMessage(error))
    }
  }, [editorialState, invalidate])
  ...
  return <section className="min-w-0 space-y-5 p-4 md:p-6" aria-labelledby="inbox-heading">
    ...
    {visible.length ? <div className="flex flex-wrap items-center gap-2"><Button variant="ghost" onClick={selectVisible}>Select visible page</Button><span className="text-xs text-muted-foreground">Up to {MAX_STORY_SELECTION} stories across pages.</span></div> : null}
    {selected.size ? <div className="flex flex-wrap items-center gap-2 rounded-lg border p-3" aria-live="polite"><strong>{selected.size} stories selected</strong><Button onClick={() => bulk.mutate({ ids: selectedIds, state: "shortlisted" })} disabled={bulk.isPending}>Shortlist selected</Button><Button variant="destructive" onClick={() => bulk.mutate({ ids: selectedIds, state: "rejected" })} disabled={bulk.isPending}>Reject selected</Button><Button variant="ghost" onClick={clearSelection}>Clear</Button></div> : null}
    ...
    {query.isPending && !visible.length ? <div role="status" aria-label="Loading stories">Loading stories…</div> : query.isError && !visible.length ? <div className="space-y-2"><div role="alert" dir="auto" className="text-red-700">{getApiErrorMessage(query.error, "Stories could not be loaded")}</div><Button variant="outline" onClick={() => query.refetch()}>Retry stories</Button></div> : !visible.length ? <Card><CardContent className="p-8 text-center text-muted-foreground">No grouped stories match these filters.</CardContent></Card> : <div className="space-y-3" data-testid="story-page">{visible.map((story) => <StoryRow key={story.id} story={story} focusedEvidenceId={deepLink?.storyId === story.id ? deepLink.evidenceId : null} researchRunId={boundResearch?.storyId === story.id ? boundResearch.runId : null} selected={selected.has(story.id)} onSelect={selectStory} open={openStory === story.id} onOpen={toggleStory} onResearch={researchStoryById} onState={changeStoryState} />)}</div>}
    {(nextCursor === undefined ? query.data?.nextCursor : nextCursor) ? <Button variant="outline" disabled={loadingMore} onClick={() => void loadMore()}>{loadingMore ? "Loading next page…" : "Load next page"}</Button> : null}
...
const StoryRow = memo(function StoryRow({ story, focusedEvidenceId, researchRunId, selected, onSelect, open, onOpen, onResearch, onState }: { story: StorySummary; focusedEvidenceId: string | null; researchRunId: string | null; selected: boolean; onSelect: (id: string, value: boolean) => void; open: boolean; onOpen: (id: string) => void; onResearch: (id: string) => void; onState: (id: string, state: "shortlisted" | "rejected") => Promise<void> }) {
  ...
  return <Card size="sm" data-story-row={story.id}><CardContent className="space-y-3">
    <div className="grid gap-3 md:grid-cols-[auto_minmax(0,1fr)_auto]">
      <input type="checkbox" aria-label={`Select ${story.title}`} checked={selected} onChange={(event) => onSelect(story.id, event.target.checked)} />
      <button type="button" className="min-h-11 min-w-11 text-left min-[900px]:min-h-0 min-[900px]:min-w-0" aria-expanded={open} aria-label={`Open ${story.title}`} onClick={() => onOpen(story.id)}>
    ...
    <div className="flex flex-wrap gap-2">
      <Button size="sm" variant="outline" onClick={() => void onState(story.id, "shortlisted")}>Shortlist</Button>
      <Button size="sm" variant="destructive" onClick={() => void onState(story.id, "rejected")}>Reject</Button>
      {!story.completeness.complete ? <Button size="sm" variant="outline" onClick={() => onResearch(story.id)}>Research more</Button> : null}
...
function boundedStoryPage(items: StorySummary[]) {
  return deduplicateStories(items).slice(0, STORY_INBOX_PAGE_SIZE)
}

...
export function StoryInbox({ initialStories }: { initialStories?: StorySummary[] }) {
  ...
  useEffect(() => { const query = new URLSearchParams(window.location.search); const storyId = query.get("story_id"); const runId = query.get("research_run_id"); if (storyId && runId) { setBoundResearch({ storyId, runId }); setOpenStory(storyId) } }, [])

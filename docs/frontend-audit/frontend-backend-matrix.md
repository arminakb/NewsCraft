...
| Today `/` | `GET /automation-control`, `GET /jobs`, `GET /jobs/summary`, `GET /telegram/publication-outcomes`, `GET /telegram/reconciliation` | `PATCH /automation-control`, `POST /jobs/{id}/retry`, `POST /jobs/{id}/cancel` | Primary operational overview with narrow publication outcomes and exact reconciliation cases. |
...
| Drafts `/drafts` | `GET /content-pack-requests` | None on list | Durable generation requests and completed content packs; editing and approval live only in the content-pack workspace. |
...
| Exact review `/review/[revisionId]` | Exact revision, pack, evidence, `GET /telegram/revisions/{id}/publication-context`, Telegram dispatch/destination/control state, persisted manual plan | Telegram publish/schedule; manual publication plan/checklist/complete/cancel | Editing and approval stay in the content-pack workspace. This route is the publication handoff: Telegram uses guarded durable jobs and other platforms remain manual-only. |
...
- Publication handoff: narrow publication outcome/context reads under `/telegram`, exact publish/schedule requests, publish-job status, and reconciliation.

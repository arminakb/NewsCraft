...
class AgentResult(list):
    def __init__(self, articles, report):
...
def run_news_agent(
    ...
    if "huggingface" in selected_sources:
        ...
        report["sources"]["Hugging Face"] = len(items)
        fetched.extend(items)
    if "github" in selected_sources:
        ...
        report["sources"]["GitHub"] = len(items)
        fetched.extend(items)
    if "youtube" in selected_sources:
        ...
        report["sources"]["YouTube"] = len(items)
        fetched.extend(items)
    ...
    report["saved"] = save_articles(processed)
    report["skipped_duplicates"] = max(len(processed) - report["saved"], 0)
    return AgentResult(processed, report)
